...
(() => {
  ...
  list.addEventListener("click",e=>{
    ...
    input.value=Jene.localDate();modal.showModal();
  });
  document.getElementById("payment-form").addEventListener("submit",e=>{
    e.preventDefault();if(!selected||selected.status==="Pago")return;
    selected.status="Pago";selected.paidAt=input.value;Jene.persist();modal.close();render();Jene.toast("Pagamento registrado nesta demonstração.");
  });
  render();
})();
