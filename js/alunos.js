...
(() => {
  ...
  function render(){
    const students=Jene.sortStudents(Jene.state.students.filter(s=>(category==="Todos"||s.category===category)&&normalize(s.name).includes(normalize(query))),document.getElementById("student-sort").value);
